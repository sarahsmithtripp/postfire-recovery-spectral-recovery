...
# Blue Mtns. Raster
blue.mtns.ras <- rast(here('data', 'blue_mtns_misc', 'bluemtnsras.tif'))
# Postfire Regen. Data (Davis et al)
postfire.regen <- rast(here('data', 'regen_prob', 'postfire_regen_prob.tif'))
...
## here is the summary for spectral cluster elements 
wide.cluster.df <- merge(traj.df, wide.df, by = c('postdisturbance_nbr', 'delta_nbr', 'relative_regrowth', 'pct_recovery'))

## summary metrics (for Sarah Smith-Tripps Review)
traj.df.summary <- wide.cluster.df %>% group_by(cluster) %>% 
  summarise(slope_= mean(postfire_fitted_slope), 
            sd = sd(postfire_fitted_slope))
```
...
field.cluster.sum <- field.cluster.df %>% select(-starts_with('0')) %>%
  group_by(field.cluster) %>% 
  summarise(slope_NBR = mean(postfire_fitted_slope), 
            sd_NBR = sd(postfire_fitted_slope))
```
...
y.alt <- c(0, 0, 0, 0, 0, 0, 0,
           ...
           -200, -150, -100, -75, -40, -10, 20,
           40, 60, 70, 80)
divergence.df <- data.frame(relative_year, significance, y)
...
## Stats Tests
### Clusters Diverging Over Time
```{r}
## updating this to run as a function  
## get emmeans for each year
calc_lmeremmeans <- function(df, var, year){
  year_filt <- year
  filt_df <- df %>% 
    filter(relative_year == year_filt)
  mod <- lmer(yearly_delta_nbr  ~ field.cluster + (1|fire), filt_df)
  ## convert out_emmans to df
  out_emeans <- summary(emmeans(mod, pairwise ~ field.cluster, adjust = 'tukey'))$contrasts
  diff_ <- out_emeans$p.value < 0.05
  out_sf_df <- data.frame(clust = out_emeans$contrast, 
                          est = out_emeans$estimate, 
                          SE = out_emeans$SE,
                          df = round(out_emeans$df,2),
                          p = out_emeans$p.value,
                          diff = diff_,
                          year = year)
}
## Question for joe - why is this only 9:23 ? 
sig_by_year <- lapply(1:23, function(x) calc_lmeremmeans(long.field.cluster.df, 'yearly_delta_nbr', x)) %>% bind_rows()
