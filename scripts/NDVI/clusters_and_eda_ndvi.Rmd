...
## summary metircs 
spect_summ_slopes <- wide.cluster.df %>% group_by(cluster) %>% 
  summarise(slope = mean(postfire_fitted_slope),
...
## get summary data for Sarah Smith-Tripps Review 
field_clust_sums <-field.cluster.df %>% group_by(field.cluster) %>% 
  summarise(slope_mean = mean(postfire_fitted_slope),
            slope_sd = sd(postfire_fitted_slope))
field_clust_sums
```
